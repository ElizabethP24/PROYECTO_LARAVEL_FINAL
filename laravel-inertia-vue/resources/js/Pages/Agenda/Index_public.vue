    ...
    async function fetchAvailability(id) {
        ...
        try {
            ...
            const { data } = await axios.get(`/api/doctors/${id}/availability`, { params })
    ...
    <div class="min-h-screen bg-gray-50 p-6">
        <div class="max-w-3xl w-full mx-auto text-center mb-8">
            <div class="flex justify-center mb-4">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAFfG-a7rTko_prForzsEBBZDodHz_wDSR_w&s" alt="Clinica Logo" class="h-20 w-20 object-contain" />
            </div>
            <h1 class="text-3xl font-extrabold text-gray-800 mb-3">Clínica Local SAS</h1>
            <p class="text-gray-600 mb-6">En esta página puede asignar las citas con el especialista que requiera. Seleccione la especialidad y el profesional, y confirme la franja horaria disponible.</p>
            <div class="flex items-center justify-center space-x-4">
                <Link :href="props.loginUrl || '/login'" class="px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-100">Inicio sesión (Área administrativa)</Link>
                <Link :href="props.registerUrl || '/register'" class="px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-100">Registrarse</Link>
            </div>
        </div>
                                ...
                                <div v-for="day in weekDays" :key="day.value" class="border rounded-xl p-2 text-center">
                                    ...
                                    <div v-if="availability[day.value]?.length">
                                        <button
                                            ...
                                            :key="slot"
                                            @click="() => { if (!isSlotDisabled(day.value, slot)) selectedSlot = { date: day.value, time: slot } }"
